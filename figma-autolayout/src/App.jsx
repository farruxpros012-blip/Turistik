      ...
      <div style={styles.main}>
        {!isPreview && status !== STATUS.LOADING && (
          <div style={styles.uploadCard}>
            ...
            <p style={styles.supportNote}>HTML, CSS, rasm fayllari qo'llab-quvvatlanadi</p>
          </div>
        )}
        ...
        {isPreview && (
          <div style={styles.previewLayout}>
            <div style={styles.previewBar}>
              ...
              <button onClick={reset} style={styles.resetBtn}>✕ Boshqa fayl</button>
            </div>
            ...
            <div style={styles.copySection}>
              {status === STATUS.COPIED ? (
                ...
                <button
                  style={{ ...styles.copyBtn, opacity: status === STATUS.COPYING ? 0.7 : 1 }}
                  onClick={handleCopy}
              ...
              <p style={styles.copyHint}>Bosing → Figmada Ctrl+V bilan paste qiling</p>
            </div>
          </div>
        )}
      </div>
...
const styles = {
  root: { minHeight: '100vh', background: '#1e1e1e', color: '#e5e5e5', fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", display: 'flex', flexDirection: 'column' },
  header: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderBottom: '1px solid #2c2c2c', background: '#252525' },
  logo: { display: 'flex', alignItems: 'center', gap: 8 },
  logoText: { fontSize: 15, fontWeight: 600, color: '#e5e5e5', letterSpacing: '-0.3px' },
  headerSub: { fontSize: 11, color: '#666', background: '#2c2c2c', padding: '3px 8px', borderRadius: 4 },
  main: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 },
  uploadCard: { width: '100%', maxWidth: 480, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 },
  dropZone: { width: '100%', border: '2px dashed', borderRadius: 16, padding: '48px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, cursor: 'default', transition: 'all 0.2s ease' },
  uploadIcon: { marginBottom: 8, opacity: 0.8 },
  uploadTitle: { fontSize: 16, fontWeight: 500, color: '#e5e5e5', margin: 0 },
  uploadHint: { fontSize: 13, color: '#666', margin: 0 },
  uploadBtn: { marginTop: 8, padding: '10px 24px', background: '#18a0fb', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'inherit' },
  supportNote: { fontSize: 11, color: '#555', margin: 0 },
  centered: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 },
  spinner: { width: 36, height: 36, border: '3px solid #2c2c2c', borderTop: '3px solid #18a0fb', borderRadius: '50%', animation: 'spin 0.8s linear infinite' },
  loadingText: { fontSize: 14, color: '#8c8c8c', margin: 0 },
  previewLayout: { width: '100%', maxWidth: 900, display: 'flex', flexDirection: 'column', background: '#252525', borderRadius: 12, border: '1px solid #333', overflow: 'hidden' },
  previewBar: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderBottom: '1px solid #333', background: '#2c2c2c' },
  fileTag: { display: 'flex', alignItems: 'center', gap: 6 },
  fileTagText: { fontSize: 12, color: '#8c8c8c', fontFamily: 'monospace' },
  resetBtn: { background: 'none', border: '1px solid #3d3d3d', borderRadius: 6, color: '#666', fontSize: 11, padding: '4px 10px', cursor: 'pointer', fontFamily: 'inherit' },
  previewFrame: { background: '#fff', minHeight: 320, maxHeight: 500, overflow: 'hidden' },
  iframe: { width: '100%', height: 420, border: 'none', display: 'block' },
  copySection: { padding: '20px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, borderTop: '1px solid #333' },
  copyBtn: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '14px 40px', background: '#18a0fb', color: '#fff', border: 'none', borderRadius: 10, fontSize: 16, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', minWidth: 180 },
  btnSpinner: { display: 'inline-block', width: 14, height: 14, border: '2px solid rgba(255,255,255,0.3)', borderTop: '2px solid #fff', borderRadius: '50%', animation: 'spin 0.7s linear infinite', marginRight: 8 },
  copyHint: { fontSize: 12, color: '#555', margin: 0, textAlign: 'center' },
  copiedBox: { display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(27,196,125,0.1)', border: '1px solid rgba(27,196,125,0.3)', borderRadius: 10, padding: '12px 24px' },
  copiedIcon: { fontSize: 24, color: '#1bc47d' },
  copiedTitle: { fontSize: 15, fontWeight: 600, color: '#1bc47d', margin: '0 0 2px' },
  copiedHint: { fontSize: 12, color: '#8c8c8c', margin: 0 },
};
